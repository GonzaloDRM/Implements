...
public class JoinSpecification {
    ...
    public static Specification<Objeto> hasAssetType(Integer objetoId) {
        return (root, query, criteriaBuilder) -> { 
            ...
            Join<Objeto, Objeto2> objeto2Join = subqueryRoot.join(Objeto_.Objeto2);
    ...
    public static Specification<Objeto> hasProjectId(Integer objetoId) {
        return (root, query, criteriaBuilder) -> {
            ...
            Root<Objeto> subqueryRoot = subquery.from(Objeto.class);
            query.distinct(true);
            subquery.select(subqueryRoot.get(SurveyQuestion_.ID));
            
            Join<Objeto, Objeto2> projectRoot = subqueryRoot.join(Objeto_.OBJETO2, JoinType.INNER);
